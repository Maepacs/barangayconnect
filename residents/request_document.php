...
$tracking_number = sprintf(
  ...
  $handled_by = null;
    ...
    if ($stmt->execute()) {
        ...
        $nStmt = $conn->prepare("INSERT INTO notifications (user_id, type, ref_id, title, message) VALUES (?, 'document', ?, ?, ?)");
        $notifTitle = 'Document Request Submitted';
        $notifMsg = 'Your request for ' . $document . ' was submitted. Tracking: ' . $submitted_tracking_number;
